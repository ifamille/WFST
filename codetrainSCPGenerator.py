...
path = './voxforge/train/wav/'
destination_directory = './voxforge/manual/'
...
if os.path.exists(path):
    ...
    for file_or_folder in os.listdir(path):
        ...
        if os.path.isfile(wav_file) and f_extension == 'wav':
            fw.write('../train/wav/' + str(file_or_folder))
            fw.write(' ')
            fw.write('../train/mfcc/' + str(f_name) + '.mfc')
            fw.write('\n')
            fw_beta.write('../train/mfcc/' + str(f_name) + '.mfc')
            fw_beta.write('\n')
else:
    print('Wrong path!')
